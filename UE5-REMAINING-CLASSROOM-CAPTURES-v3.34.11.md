# UE5 Learning Hub v3.34.11 — Genuine Remaining Classroom Captures

This list is deliberately stricter than v3.34.10. A teacher capture is listed only when the student still needs a **specific classroom Unreal editor state/graph/configuration** that is not already covered by an exact current Epic UE5.8 visual. Simple repeatable editor actions now reuse official UE5.8 UI, and proof/test steps are not counted as screenshots.

- Core Lesson classroom capture groups: **17**.
- Programming / Quick Tutorial classroom capture groups: **53**.
- Building Block recognition captures: **6**.
...
|---|---|---|
| `UE5-lesson-blueprint-classes-setup.png` | Blueprint Classes & Instances: Reuse Without Duplication | **Step 5:** Return to the level. Select Block_A and set BlockLabel to A; Block_B to B; Block_C to C. **Step 6:** Move the three instances to visibly different positions. Do not duplicate the Blueprint asset itself. |
| `UE5-lesson-events-logic.png` | Events & Execution: Make Logic Actually Run | **Step 4:** Connect a third Print String: 03 Third, Duration 2.0. Compile. |
| `UE5-lesson-branches-logic.png` | Branches: Decisions & Conditions | **Step 2:** Event BeginPlay → Get Health → add Float <= Float. Type 0.0 into the second input. **Step 4:** True → Print String “Dead”. False → Print String “Still alive”. Compile. **Step 8:** Add a comment around the comparison + Branch: “Is Health zero or below?” so the graph reads like a question. |
| `UE5-lesson-functions-logic.png` | Functions & Macros: Reuse Logic Clearly | **Step 3:** Inside the function add Print String and connect Message to In String. **Step 6:** Add a Custom Event named TestSecondCall → call ShowStatus with Message Door unlocked. |
...
| `UE5-lesson-interfaces-dispatchers-logic.png` | Interfaces & Event Dispatchers: Communicate Cleanly | **Step 4:** In BP_InterfaceDoor add the Event Interact implementation → Print String “Door interacted”. Compile. |
| `UE5-lesson-collision-logic.png` | Collision & Overlaps: Detect the World Reliably | **Step 2:** Select Box Collision → set Box Extent X 100, Y 100, Z 100. **Step 7:** Use Other Actor → Equal (Object) with Get Player Character if you want to prove only the player should trigger it. |
...
| `UE5-lesson-ui-ui.png` | UMG & Gameplay UI: Show State Without Owning It | **Step 3:** Add Progress Bar at X 40, Y 80. Set width 300 and height 24. **Step 5:** Inside SetHealthDisplay calculate CurrentHealth / MaxHealth and Set Percent on the Progress Bar. |
...
| `UE5-lesson-animation-logic.png` | Animation Blueprints & State Machines | **Step 1:** Open the Animation Blueprint used by the character. In Event Graph find Event Blueprint Update Animation. **Step 3:** Try Get Pawn Owner → Get Velocity → Vector Length XY → Set Speed. **Step 4:** From the pawn/Character Movement → Is Falling → Set IsInAir. **Step 6:** Set Idle → Run rule Speed > 5. Set Run → Idle rule Speed <= 5. |
...
| `UE5-tutorial-dash-setup.png` | Make a Simple Dash | **Step 4:** Add Launch Character. Connect the scaled vector to Launch Velocity. Enable XY Override; leave Z Override off. |
