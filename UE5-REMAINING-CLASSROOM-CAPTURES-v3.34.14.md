# UE5 Learning Hub v3.34.14 — Genuine Remaining Classroom Captures

This list is deliberately strict. A teacher capture is listed only when the student still needs a **specific classroom Unreal editor state/graph/configuration** that is not already covered by an accurate official Epic UE5.6, UE5.7 or UE5.8 visual. UE5.8 remains the technical source of truth; 5.6/5.7 imagery is accepted only when the shown workflow and controls remain materially unchanged. Proof/test steps are not counted as screenshots.

- Core Lesson classroom capture groups: **8**.
- Programming / Quick Tutorial classroom capture groups: **49**.
...
- **Total genuine teacher screenshots: 76**
- Building Block diagrams remain **7** and are not teacher screenshots.

This is down from **77** in v3.34.13, **80** in v3.34.12, **90** in v3.34.11 and from the original **211**-capture audit.

## What no longer counts as a teacher capture

- Create Blueprint / choose parent class: accurate official Epic UE5.6–5.8 visual is reused.
- Add a component: accurate official Epic UE5.6–5.8 Add Component visual is reused.
- Create simple variables: accurate official Epic UE5.6–5.8 My Blueprint variable visual is reused.
- Create Input Actions / recognise Enhanced Input events: accurate official Epic UE5.6–5.8 visuals are reused.
- Create basic Widget assets / recognise common HUD widgets: accurate official Epic UE5.6–5.8 UMG visuals are reused.
...
- Line Trace camera/vector maths, Break Hit Result/Hit Actor and Branch recognition use exact official Epic visuals; Hub-specific distances remain written beside the images.
- Niagara Impact Point / Impact Normal recognition now reuses Epic's exact Break Hit Result visual, so a separate teacher screenshot for that step is no longer needed.

## Capture standard

- Current classroom UE build.
- Official Epic UE5.8 imagery first; UE5.7 and UE5.6 imagery is also acceptable when the shown UI/workflow is still materially accurate.
- Do not use an older visual if a control has moved, been renamed or the workflow has changed.
...
| `UE5-block-niagara-systems.png` | Niagara Systems & Spawning VFX | **Narrowed capture:** the Hub now supplies Epic's official Niagara Editor UI visual for System Overview/emitter recognition. Capture only the Hub-specific Blueprint proof: BeginPlay → Spawn System at Location → System Template = NS_BlockTest → Location = Get Actor Location. |
