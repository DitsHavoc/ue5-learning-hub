# Current release — v3.37.1

**Topology Step Mockups.** Crate, Barrel / Canister and Sci-fi Wall Panel now prototype the next visual teaching layer: a **READ THE MESH** topology map before modelling begins plus a wireframe/edge-placement diagram inside every step. This is intentionally a three-page proof-of-pattern before rolling the approach across all eight Build X projects. Existing lesson/build/fix IDs and progress remain unchanged; no database migration is required. See `RELEASE-NOTES-v3.37.1.md` and `3D-MODELLING-TOPOLOGY-MOCKUP-QA-v3.37.1.md`.
