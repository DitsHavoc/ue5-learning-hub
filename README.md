# Current release — v3.37.2

**Split Viewport Step Visuals.** The approved low-poly Max-style crate, barrel and sci-fi panel workflows now work as actual teaching guides rather than one overview image: each build keeps a full roadmap near the top, while the modelling walkthrough shows the matching isolated viewport stage directly beside the step that creates it. Crate has also been split into an earlier **main cuts** step before panel shaping so students can see the mesh grow gradually. Existing lesson/build/fix IDs and progress remain unchanged; no database migration is required. See `RELEASE-NOTES-v3.37.2.md` and `3D-MODELLING-SPLIT-VISUALS-QA-v3.37.2.md`.
