...
**3D Modelling Studio Rebuild.** The 3ds Max area has been rebuilt around **REFERENCE → PLAN → BLOCK → MODEL → INSPECT → GAME READY → CRITIQUE** instead of presenting modelling as a sequence of software buttons. All 12 lesson IDs, 8 Build X IDs and 6 Fix This Model IDs are preserved, so existing completion/progress records continue to work. Every core lesson now begins with an explicit modelling question, observation prompts, planning decisions and a judgement call before the step-by-step Max guide. Every Build X now behaves like a small production brief with real-world reference imagery, pre-Max planning, a design variation, quality gates, a professional artist breakdown and a Critique Board handoff. The existing user-supplied 3ds Max reference figures remain supporting material; current Autodesk, Adobe and Epic documentation remain the source of truth for current UI/pipeline behaviour. No database migration is required. See `RELEASE-NOTES-v3.37.0.md`, `3D-MODELLING-STUDIO-REVAMP-v3.37.0.md` and `3D-MODELLING-STUDIO-QA-v3.37.0.md`.
