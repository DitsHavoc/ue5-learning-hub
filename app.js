...
function designModulePage(id){
  ...
  ${m.id==='landscape'?`<section class="snippet-programming-cta integrated"><div><span class="eyebrow">⌘ ENGINE WORKFLOW • UNREAL LEARNING</span><h2>Need the exact Landscape buttons and settings?</h2><p>This Designer Studio page is about terrain readability, routes, atmosphere and environment decisions. The complete UE5.8 tool workflow lives once in Unreal Learning so we do not maintain two conflicting versions.</p></div><a class="button primary" href="#/tutorial/landscape-master-guide">Open Landscape Master Guide →</a></section>`:''}
  ${m.id==='cinematics'?`<section class="snippet-programming-cta integrated"><div><span class="eyebrow">⌘ ENGINE WORKFLOW • UNREAL LEARNING</span><h2>Need the exact Sequencer buttons, tracks and render steps?</h2><p>This Designer Studio page is about shot purpose, framing, pacing and visual storytelling. The technical UE5.8 workflow lives once in Unreal Learning: Level Sequences, keys, Cine Cameras, Camera Cuts, focus and Movie Render Queue.</p></div><a class="button primary" href="#/tutorial/sequencer-master-guide">Open Sequencer Master Guide →</a></section>`:''}
...
function tutorialRevisionBridge(t){
  ...
  if(t.designModule)return `<section class="content-card tutorial-revision-bridge practical"><div><span class="eyebrow">PRACTICAL PROOF • NO DUPLICATE QUIZ</span><h2>Build this; revise the design judgement.</h2><p>This recipe is assessed by whether the result works and you can explain the decision. The parent Designer Studio topic holds the scenario questions.</p></div><button class="button ghost" data-action="revision-topic-start" data-revision-topic-id="design:${esc(t.designModule)}" data-revision-count="5">Revise ${esc(designModule(t.designModule)?.title||'Designer Studio')} →</button></section>`;
...
function tutorialPage(id){
  const t=tutorial(id);if(!t)return notFound();const c=tutorialCategory(t.category),done=tutorialDone(t.id),ref=tutorialOfficialRef(t),f=t.familyId?tutorialFamily(t.familyId):null,unrealGuide=!!t.unrealHome,home=unrealGuide?'<a href="#/programming">Unreal Learning</a>':t.libraryHidden?`<a href="#/lesson/${t.lessonHome||t.referenceLesson}">${esc(lesson(t.lessonHome||t.referenceLesson)?.title||'Unreal Learning')}</a>`:(f&&familyTutorials(f).length>1?`<a href="#/tutorial-family/${f.id}">${esc(f.title)}</a>`:'<a href="#/tutorials">Quick Tutorials</a>'),related=unrealGuide?[]:(f?familyTutorials(f).filter(x=>x.id!==t.id):TOOLS.tutorials.filter(x=>x.id!==t.id&&!x.libraryHidden&&(x.category===t.category||x.uses.some(u=>t.uses.includes(u)))).slice(0,4));
  const designMode=!!t.designModule,unrealSection=t.unrealSection||'Engine Workflow',categoryLabel=unrealGuide?unrealSection:(c?.title||t.category),categoryIcon=unrealGuide?(t.icon||'⌘'):(c?.icon||'🛠');
...
function programmingPage(){
  ...
  <section class="section master-guides-programming" id="master-guides"><div class="section-head"><div><span class="eyebrow">03 • FULL WORKFLOWS</span><h2>🧭 Unreal Master Guides</h2><p>These are part of Unreal Learning — not separate mini-sites. Each guide has one authoritative technical workflow, a Designer Studio bridge and a focused Revision check.</p></div><a class="button ghost" href="#/tutorials">See Quick Tutorials →</a></div><div class="master-guide-grid">${guides.map(masterGuideCard).join('')}</div></section>
...
function route(options={}){
  ...
  else if(parts[0]==='tutorial'&&parts[1]){const t=tutorial(parts[1]);app.innerHTML=tutorialPage(parts[1]);activate(t?.unrealHome?'programming':'tutorials')}
