...
function designCaseStudyCard(c,i){
  const source=c.sourceUrl?`<a class="designer-case-source" href="${esc(c.sourceUrl)}" target="_blank" rel="noopener">Source: ${esc(c.sourceTitle||c.game)} ↗</a>`:'';
  return `<article class="designer-case-card"><div class="designer-case-image">${zoomableImage({src:c.image,alt:`${c.game} design case study`,caption:`${c.game} • ${c.lens}`,sourceUrl:c.sourceUrl||'',sourceTitle:c.sourceTitle||c.game,kind:'reference',eager:i===0})}</div><div class="designer-case-body"><span class="eyebrow">CASE STUDY ${String(i+1).padStart(2,'0')} • ${esc(c.lens||'DESIGN READ')}</span><h3>${esc(c.game)}</h3><div class="designer-case-read"><b>READ IT</b><p>${esc(c.notice)}</p></div><div class="designer-case-steal"><b>STEAL THE PRINCIPLE</b><p>${esc(c.steal)}</p></div><div class="designer-case-task"><b>2-MINUTE TASK</b><p>${esc(c.task)}</p></div>${source}</div></article>`;
}
...
function designPage(){
  ...
  const researchCount=DESIGN.modules.reduce((n,m)=>n+(m.researchMissions||[]).length,0),challengeCount=(DESIGN.roulette||[]).length,gameCount=new Set(DESIGN.modules.flatMap(m=>(m.caseStudies||[]).map(c=>c.game))).size;
  ...
  <section class="resource-studio-bridge"><div><span class="deep-label">🧰 FREE ASSETS • SOUND • PROFESSIONAL REFERENCE</span><h2>Don't bury the useful stuff at the bottom.</h2><p>The Resource Library collects licence-clear assets, professional sound, game-level explorers, UI reference, documentaries, talks and postmortems in one place. Use it whenever the design needs a better placeholder or a stronger reference.</p><div class="portal-chip-row"><span>ambientCG</span><span>noclip.website</span><span>Sonniss</span><span>Noclip docs</span><span>GDC</span></div></div><a class="button primary" href="#/resources">🧰 Open Resource Library →</a></section>
  ...
  <section class="section"><div class="section-head"><div><span class="eyebrow">8 DISCIPLINES • 24 DIFFERENT GAMES</span><h2>Choose the design problem you are trying to solve</h2><p>Each discipline now uses three different games in its LOOK stage — 24 distinct case studies across the Studio — before moving into developer commentary, field research, UE5 recipes and the Studio Build.</p></div></div><div class="designer-module-grid">${DESIGN.modules.map(designModuleCard).join('')}</div></section>
...
document.addEventListener('click',async e=>{
  ...
  else if(a==='design-roulette'){
    const pool=DESIGN.roulette||[];if(!pool.length)return;
    const x=pool[Math.floor(Math.random()*pool.length)],box=$('#designRouletteResult');if(!box)return;
    box.classList.add('active');box.innerHTML=`<span>${x.icon||'◆'}</span><div><b>${esc(x.moduleTitle)} — ${esc(x.title)}</b><p><strong>Rule:</strong> ${esc(x.constraint)}<br><strong>Win:</strong> ${esc(x.goal)}</p></div>`;
  }
