...
function designPage(){
  ...
  <section class="section designer-resources"><div class="section-head"><div><span class="eyebrow">BUILD FAST • STUDY DEEPLY</span><h2>Free assets, sound and professional reference</h2><p>Use placeholders aggressively when the lesson is design. Save the three-week chair model for a modelling lesson.</p></div></div><div class="designer-resource-grid">${DESIGN.resources.map(designResourceCard).join('')}</div><div class="callout"><b>Reference habit:</b> screenshots are evidence, not decoration. Save the source, write what you are studying, and copy the design principle rather than somebody else's scene.</div></section>`;
}
