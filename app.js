...
function level4SpecialistProjectPage(id){
 ...
 const prompts=(p.storyPrompts||[]).length?`<section class="level4-section"><div class="level4-section-head"><div><h2>Need a starting idea?</h2><p>Choose one of these or invent your own. The point is not the prompt — it is how clearly the environment communicates it.</p></div></div><div class="level4-prompt-grid">${p.storyPrompts.map(x=>`<article><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p></article>`).join('')}</div></section>`:'';
