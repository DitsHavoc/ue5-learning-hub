...
function buildGlobalSearchIndex(){
  ...
  (TOOLS.tutorials||[]).forEach(t=>{const f=t.familyId?tutorialFamily(t.familyId):null;add({
    title:t.title,meta:t.unrealHome?`Unreal Learning • Landscape & World Building • ${t.duration||''}`:t.libraryHidden?`Core lesson application • ${lesson(t.lessonHome||t.referenceLesson)?.title||'Unreal Learning'}`:`Practical build • ${f?.title||tutorialCategory(t.category)?.title||'UE5'} • ${t.duration||''}`,
